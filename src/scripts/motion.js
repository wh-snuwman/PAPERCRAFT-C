
import  { state }  from './init.js'
import { phi, wing } from "./api.js"


window.motion = class {
    constructor(type='') {
        this.type = type;
        this.retObj = phi.obj(null,[0,0],[0,0]);

        this.isFlip = false;
        this.frame = null;
        this.sinN = 0;
        this.onHand = false;
        this.isMove = false;
        this.interaction = false;
        this.leftKey = false;
        this.rightKey = false;
        this.click_l = false;
        this.click_r = false;
        this.press_l = false;
        this.press_r = false;
        // phi.mousepos = [0,0];
        this.isAttack = false;
        this.attackCancelTime = 0;
        this.rotate = 0;
        this.haveGun = false;
        this.gunFireDelay = 0;
        this.action = null;
        this.delay = 0
        this.delay_flag = false
        this.frameFlop = false
    }

    _devtest(pos){
        this.retObj = phi.obj(IMG.PLAYER[0],pos)
        phi.reSizeBy(this.retObj,0.7*tileRatio,'default');
        return this.retObj
    }

    render(pos,data={Rk:false,Lk:false,isMv:false,CL:false,CR:false,PR:false,PL:false,mousePos:false,haveGun:false,action:null}){
        this.rightKey = data.Rk
        this.leftKey = data.Lk
        this.isMove = data.isMv
        this.click_l = data.CL
        this.click_r = data.CR
        this.press_l = data.PL
        this.press_r = data.PR
        this.haveGun = data.haveGun
        this.action = data.action
        
        if (this.click_l && this.haveGun){
            this.attackCancelTime = Date.now() + 1200
            this.isAttack = true
        }
        if (this.attackCancelTime < Date.now()){
            this.isAttack = false
        }
        if (!this.isAttack){
            if (this.leftKey) this.isFlip = 1;
            if (this.rightKey) this.isFlip = 0;
        }

        
        if (this.action == 'attack'){
            if (phi.mousepos[0] < pos[0]){
                this.isFlip = 1
            } else {
                this.isFlip = 0
            }
            if (this.isMove && !(this.rightKey && this.leftKey)){
                this.sinN++;
                this.retObj = phi.obj(IMG.PLAYER[7],pos)
                this.frame = 7
                this.rotate = Math.sin(this.sinN/7)*5
                phi.moveY(this.retObj,Math.cos(this.sinN/3.5)*5)

            } else {
                this.rotate += (0 - this.rotate) /10

                if (this.click_l){
                    if (this.gunFireDelay < Date.now()){
                        if (this.isFlip){
                            this.rotate += 7
                        } else {
                            this.rotate -= 7
                        } 
                        this.gunFireDelay = Date.now() + 100
                    }
                }
                if (Math.abs(this.rotate) > 2){
                    this.retObj = phi.obj(IMG.PLAYER[7],pos)
                    this.frame = 7
                } else {
                    this.retObj = phi.obj(IMG.PLAYER[6],pos)
                    this.frame = 6
                }
            }
        }
        else if (this.action == 'destroy'){
            if (this.isMove && !(this.rightKey && this.leftKey)){
                this.sinN++;
                this.frame = 1
                this.retObj = phi.obj(IMG.PLAYER[1],pos)
                this.rotate += (0 - this.rotate) /10
                this.rotate = Math.sin(this.sinN/7)*5
                phi.moveY(this.retObj,Math.cos(this.sinN/3.5)*5)
            } else {
                if (phi.mousepos[0] < pos[0]){
                    this.isFlip = 1
                } else {
                    this.isFlip = 0
                }

                if (this.click_l){
                    if (!this.delay_flag){
                        this.delay = Date.now()
                        this.delay_flag = true
                    }
                }

                if (this.press_l && this.delay_flag){
                    if ((Date.now() - this.delay)  > 200){
                        this.delay = Date.now()
                        this.frameFlop = !this.frameFlop
                    }
                } else {
                    this.delay = 0
                    this.delay_flag = false
                    this.frameFlop = false
                }

                if (this.isFlip){

                    if (this.frameFlop){
                        this.retObj = phi.obj(IMG.PLAYER[4],[pos[0]-38,pos[1]])
                        this.frame = 4
                    } else {
                        this.retObj = phi.obj(IMG.PLAYER[0],[pos[0],pos[1]])
                        this.frame = 0 
                    }

                } else {
                    if (this.frameFlop){
                        this.retObj = phi.obj(IMG.PLAYER[4],pos)
                        this.frame = 4
                    } else {
                        this.retObj = phi.obj(IMG.PLAYER[0],pos)
                        this.frame = 0 
                    }
                }

                this.rotate += (0 - this.rotate) /10

            }
        } 
        else if (this.action == 'default' || this.action == 'build'){
            if (this.isMove && !(this.rightKey && this.leftKey)){
                this.sinN++;
                if (this.onHand){
                    this.retObj = phi.obj(IMG.PLAYER[3],pos)
                    this.frame = 3
                } else {
                    this.retObj = phi.obj(IMG.PLAYER[1],pos)
                    this.frame = 1
                    this.rotate += (0 - this.rotate) /10
                }
                this.rotate = Math.sin(this.sinN/7)*5
                phi.moveY(this.retObj,Math.cos(this.sinN/3.5)*5)
            } else {
                if (this.onHand){
                    this.retObj = phi.obj(IMG.PLAYER[2],pos)
                    this.frame = 2
                } else {
                    this.retObj = phi.obj(IMG.PLAYER[0],pos)
                    this.rotate += (0 - this.rotate) /10
                    this.frame = 0
                }
            }
        }
        
        if (this.isFlip){phi.flip(this.retObj,'hor')}// 텍스쳐반전 
        phi.rotate(this.retObj,this.rotate)
        phi.reSizeBy(this.retObj,0.7 * state.tileRatio,'default');
        return this.retObj

    }


    renderOther(pos,frame,isFlip,isMove){
        this.retObj = phi.obj(IMG.PLAYER[frame],pos)
        if (isMove){
            this.sinN++;
            this.rotate += (0 - this.rotate) /10
            this.rotate = Math.sin(this.sinN/7)*5
            phi.moveY(this.retObj,Math.cos(this.sinN/3.5)*5)   
        } else {
            this.rotate = 0
            this.sinN = 0
        }

        if (isFlip){phi.flip(this.retObj,'hor')}
        phi.rotate(this.retObj,this.rotate)
        phi.reSizeBy(this.retObj,0.7 * state.tileRatio,'default');
        return this.retObj

    }

    renderNotPLayer(type,pos,isFlip,isMove){
        if (type == 'soldier'){
            if (isMove){
                this.retObj = phi.obj(IMG.ENTITY['soldier'][1],pos)
            } else {
                this.retObj = phi.obj(IMG.ENTITY['soldier'][0],pos)
            }


            if (isMove){
                this.sinN++;
                this.rotate += (0 - this.rotate) /10
                this.rotate = Math.sin(this.sinN/7)*5
                // if isMove
                phi.moveY(this.retObj,Math.cos(this.sinN/3.5)*5)   
            } else {
                this.rotate = 0
                this.sinN = 0
            }
    
            if (isFlip){phi.flip(this.retObj,'hor')}
            phi.rotate(this.retObj,this.rotate)
            phi.reSizeBy(this.retObj,0.7 * tileRatio,'default');
            return this.retObj
        }

    }
}